...
class HomeController extends Controller {
	...
	* Show the application dashboard.
	...
	public function getScore($match_id){
		...
		$homeScore = $scoreData[0]->event->score->home;
		$awayScore = $scoreData[0]->event->score->away;
	...
	public function saveData() {
		// echo $match_api_id."<br>";
		$k = json_decode(crawlData());
		$all_match = $k->mod->d[0]->c;
        $dumpData="";
		foreach ($all_match as $match) {
			$match_api_id = $match->e[0]->k;
			// echo $match_api_id."||<br>";
			$scoreData =getScrore($match_api_id);
			// dd($homeScore."-".$awayScore);
